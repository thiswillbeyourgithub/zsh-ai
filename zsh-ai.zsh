...
fzf_ai_commands() {
  ...
  ZSH_AI_GPT_SYSTEM="You only answer up to $ZSH_AI_N_GENERATIONS appropriate shell one liner that does what the user asks for. The user is using the $(basename $SHELL) shell and his setup infos are '$(uname --kernel-name --kernel-release --kernel-version)'. Today's date is '$(date "+%Y-%m-%d (%A, %B %d, %Y)")'. You answer using structured output. If your answer uses arguments or flags, you MUST include a short paragraph to explain each (do omit self explanatory placeholders like <ip> or <serverport>). Be careful to properly escape things because I will parse your answer expecting json, especially newlines, pipes, etc. You have to answer a unique 'items' key whose value is a list of dict with keys 'json_escaped_code' and 'json_escaped_explanation'. Your code can only be one liners otherwise my parsing of your output will fail! So only use a single paragraph without newlines in your explainer. For the explainer you can specify newlines using '<br>' though, I will replace them by a newline for readability so you can format one argument per line for example. Phrase it as military documentation, so very much to the point (i.e. don't start by 'this command blabla'). It is VERY important to make your answer appear on a single line otherwise it will not appear in the fzf I will call in your output."
